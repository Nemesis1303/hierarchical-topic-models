...
path_models_ctm = pathlib.Path(
    "/export/usuarios_ml4ds/lbartolome/Datasets/CORDIS/models_htm_ctm")
path_models_mallet = pathlib.Path(
    "/export/usuarios_ml4ds/lbartolome/Datasets/CORDIS/models_htm")
...
for el in df.iter.unique():
    ...
    for entry in path_root.iterdir():
        if entry.joinpath('TMmodel/alphas.npy').is_file():
            ...
            # Get thr if htm-ds
            thr = 0 if "ws" in entry.as_posix() else float(
                entry.as_posix().split("thr_")[1].split("_")[0])

            # Get topic from which the submodel is generated
            exp_tpc = int(entry.as_posix().split(
                "from_topic_")[1].split("_")[0])
