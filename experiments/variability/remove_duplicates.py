    ...
    if root_topics:
        ...
        for root_topic in root_topics:
            ...
            for entry in path_models_tpc.iterdir():
                ...
                models_date = [int(x.split("_")[-1][-2:])
                               for x in directories_list]
                models_name = ["_".join(x.split("_")[:-1])
                               for x in directories_list]
                        ...
                        if i != j:
                            if model_name == model_name_:
                                if models_date[i] > models_date[j]:
                                    try:
                                        shutil.rmtree(entry.joinpath(
                                            directories_list[j]))
                                        print("Removing", entry.joinpath(
                                            directories_list[j]))
                                        print("Keeping", entry.joinpath(
                                            directories_list[i]))
                                    except:
                                        print("Could not be removed", entry.joinpath(
                                            directories_list[j]))
        ...
        path_models_tpc = pathlib.Path(path_models)
                            ...
                            if models_date[i] > models_date[j]:
                                try:
                                    ...
                                    print("Could not be removed",
                                          entry.joinpath(directories_list[j]))
                                ...
                                try:
                                    ...
                                    print("Could not be removed",
                                          entry.joinpath(directories_list[i]))
