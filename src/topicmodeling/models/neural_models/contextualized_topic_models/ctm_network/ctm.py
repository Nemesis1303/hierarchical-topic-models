...
import numpy as np
import torch
from models.neural_models.contextualized_topic_models.ctm_network.decoding_network import \
    DecoderNetwork
...
class CTM:
    ...
    def __init__(
        ...
        assert (
            isinstance(num_data_loader_workers,
                       int) and num_data_loader_workers >= 0
    ...
    def get_doc_topic_distribution(self, dataset, n_samples=20):
        ...
        with torch.no_grad():
            for batch_samples in tqdm(loader):
                ...
                # forward pass
                self.model.zero_grad()
                mu, log_var = self.model.get_posterior(
                    X_bow, X_contextual, labels)
                thetas = self.model.sample(
                    mu, log_var, n_samples).cpu().numpy()
    ...
    def get_word_distribution_by_topic_id(self, topic):
        ...
        if topic >= self.n_components:
            ...
            t = [(word, wd[topic][idx])
                 for idx, word in self.idx2token.items()]
    ...
    def get_top_documents_per_topic_id(
        ...
        for i in ind:
            res.append(
                (unpreprocessed_corpus[i],
                 document_topic_distributions[i][topic_id])
            )
        return res
